...
public:
	...
	const int calculatePU(const int& userID, const NiImageStruct& niImageStruct,
						  const double& w1, const double& w2, const double& w3, const double& w4, const double& w5);
...
private:
	...
	vector< vector< double > > userPUHistory;	// For smoothing PU
	vector< double > userDynamics;				// For computing max hand speed over intervalMax
	vector< double > userHandSpeedMax;
	vector< vector< double > > userHandSpeedHistory;
	time_t startHS, endHS;						// For evaluating the intervalMax
};
