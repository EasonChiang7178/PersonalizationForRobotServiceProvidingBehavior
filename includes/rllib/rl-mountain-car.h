...
#ifndef rlMOUNTAIN_CAR_H
#define rlMOUNTAIN_CAR_H
...
namespace rl {
  namespace problem {
	...
	void random(void) {
	  position = rl::random::toss(param_type::minPosition(),param_type::maxPosition());
	  speed    = rl::random::toss(param_type::minSpeed(),param_type::maxSpeed());
	}
	...
	void timeStep(const action_type& a) {
	  ...
	  default:
	    std::ostringstream ostr;
	    ostr << "mountain_car::Simulator::timeStep(" << a << ")";
	...
	template<typename ARGMAX_SA_CRITIC, typename AGENT>
	static void Qdata(std::ostream& file, 
			  const ARGMAX_SA_CRITIC& critic,
			  const AGENT& agent,
			  int points_per_side,
			  bool draw_q) {
	  ...
	  for(s=0;s<points_per_side;++s) {
	    ...
	    for(p=0;p<points_per_side;++p) {
	      ...
	      a = agent.policy(current);
	      q = critic(current,a);
	      if(draw_q)
		file << position << ' ' << speed << ' ' << q << ' ' << a << std::endl;
	      else
		file << position << ' ' << speed << ' ' << a << std::endl;
	    }
	    file << std::endl;
	  }
	}
	...
	static void drawQ(std::string title,
	  ...
	  file << "unset hidden3d;" << std::endl
	       ...
	       << "set cblabel \"none=" <<  actionNone
	       << ", forward=" << actionForward
	       << ", backward=" << actionBackward
	       << "\";" << std::endl
	  ...
	  Qdata(file,critic,agent,points_per_side,true);
	  file.close();
	...
	static void drawEpisode(std::string title,
	  ...
	  double cumrew;
	  int episode_length;
	  std::vector<rl::sa::Transition<typename AGENT::state_type,
					 typename SIMULATOR::action_type,
					 typename SIMULATOR::reward_type> > transitions;
	  typename std::vector<rl::sa::Transition<typename AGENT::state_type,
						  typename SIMULATOR::action_type,
						  typename SIMULATOR::reward_type> >::iterator iter,iter_end;
	  ...
	  rl::episode::sa::run_and_collect(simulator,agent,transitions,
					   max_episode_length,episode_length);
	  for(cumrew=0,iter=transitions.begin(),iter_end=transitions.end();
	      iter != iter_end;
	      ++iter)
	    cumrew += (*iter).reward();
	  ...
	  for(iter=transitions.begin(),iter_end=transitions.end();
	      iter != iter_end;
	      ++iter)
	    file << (*iter).currentState().position << ' ' 
		 << (*iter).currentState().speed    << ' '
		 << 3 << std::endl;
	  file.close();
	  std::cout << "\"" << filename << "\" generated." << std::endl;
	  ...
	  Qdata(file,critic,agent,points_per_side,false);
	  file.close();
