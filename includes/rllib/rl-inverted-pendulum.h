...
#ifndef rlINVERTED_PENDULUM_H
#define rlINVERTED_PENDULUM_H
...
namespace rl {
  namespace problem {
	...
	void random(void) {
	  angle = param_type::angleInitNoise()*rl::random::toss(-1,1);
	  speed = param_type::speedInitNoise()*rl::random::toss(-1,1);
	}
	...
	void timeStep(const action_type& a) {
	  ...
	  aa += param_type::actionNoise()*rl::random::toss(-1,1);
	  aa *= Param::strength();
