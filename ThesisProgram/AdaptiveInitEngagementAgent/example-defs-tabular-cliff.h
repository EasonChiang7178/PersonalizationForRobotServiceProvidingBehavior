...
// the cliff positions. It fits
// rl::concept::sa::DerivableArchitecture. In usual rl algorithms,
...
public:
  ...
  param_type newParameterInstance(void) const {
    // vector size is |S|*|A|
    return gsl_vector_calloc(Cliff::size*rl::problem::cliff_walking::actionSize);
  }

  output_type operator()(const param_type  theta, 
			 const input_type &sa) const {
  ...
  output_type operator()(const param_type theta, 
    ...
    return gsl_vector_get(theta,a*Cliff::size+s);
  }
  ...
  void gradient(const param_type theta, 
    ...
    gsl_vector_set_basis(grad,a*Cliff::size+s); 
  }
};
